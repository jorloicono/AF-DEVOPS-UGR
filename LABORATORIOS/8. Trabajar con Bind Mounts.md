...
```bash
sudo docker run -it --name contenedor-ro -v ~/docker_bindmount_demo/datos:/mnt/datos:ro ubuntu
```
...
```bash
sudo docker run -it -v ~/docker_bindmount_demo/datos/saludo.txt:/mnt/saludo.txt ubuntu
```
